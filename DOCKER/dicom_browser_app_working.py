...
UPLOADER_URL = f"http://{get_browser_host()}/dicomsuploader"
...
# =========================================================
# DICOM SCANNER (ENHANCED METADATA COLUMNS)
# =========================================================
@st.cache_data(show_spinner=True, ttl=18 * 3600)
def scan_dicom_folder(folder):
    images, sr_files = [], []
    cols = [
        "FilePath", "FileName", "PatientID", "StudyDate", "StudyTime",
...
def validate_block(block_data, expected_count):
    """Ensures both Overall status and all criterion radios are filled for a block."""
...
# Key used for reading/writing persistent QC records
...
def render_qc_forms_panel(study_uid, patient_id, display_date, file_items, study_sr, record_key):
    ...
    # Default preset doctor list
    DOCTOR_MAP = {"00": "", "01": "Dr. A", "02": "Dr. B", "03": "Dr. C"}
    doctor_options = list(DOCTOR_MAP.values())

    # 1. Extract raw candidate fields from the study DataFrame
    ...
    with tab_form_01:
        with st.container(height=680, border=False):
            ...
            with col1:
                doctor_v = st.selectbox(
                    "Doctor's Name",
                    options=doctor_options,
                    key=f"{study_uid}_doc_v",
                    index=current_index
                )
            with col2:
...
with st.expander("🔍 Debug Info"):
    ...
    if not study_sr_files.empty:
        st.dataframe(study_sr_files[["FileName", "SOPInstanceUID"]])
